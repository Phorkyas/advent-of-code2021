...
def path_has_double_small_cave(path):
    multiples = [k for k,count in Counter(path).items() if count > 1]
...
def next_nodes(node, cons):
    ...
    for con in cons:
        ...
        if con[0] == node.name and con[1] != "start":
            ...
            path = node.get_path()
            if is_small(con[1]) and path_has_double_small_cave(path):
                if con[1] in path:
                    print(con[1] + " double small path in: " + str(path))
                    continue
            nodes.append(con[1])
    return nodes
